...
import {
  getConsultingCompanies,
  addConsultingCompany,
  updateConsultingCompany,
  removeConsultingCompany,
  disableConsultingCompany,
  type ConsultingCompany,
  type ConsultingAts,
} from '../../lib/api';

const ATS_OPTIONS: { value: ConsultingAts; label: string; needsSlug: boolean }[] = [
...
function atsNeedsSlug(ats: ConsultingAts): boolean {
...
function EmptyState({ onAdd }: { onAdd: () => void }) {
  return (
    <div className="flex flex-col items-center justify-center py-20 text-center">
      ...
      <h3 className="text-sm font-semibold text-gray-200 mb-1">No companies in your consulting list</h3>
      <p className="text-xs text-gray-500 mb-5 max-w-xs">
        Only companies in this list are scraped when you click <strong>Scrape Consulting Jobs</strong>.
      </p>
      <button
        onClick={onAdd}
...
function AddForm({
  ...
  onAdd: (payload: { name: string; career_url: string; ats: ConsultingAts; ats_slug?: string }) => void;
  onCancel: () => void;
  saving: boolean;
}) {
  const [name, setName] = useState('');
  const [careerUrl, setCareerUrl] = useState('');
  const [ats, setAts] = useState<ConsultingAts>('custom');
  ...
  return (
    ...
    >
      <p className="text-sm font-semibold text-gray-100 mb-4">Add Consulting company</p>
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
        ...
        <div>
          ...
          <select
            value={ats}
            onChange={(e) => setAts(e.target.value as ConsultingAts)}
...
function RowEditor({
  ...
  onSave: (id: string, patch: Partial<ConsultingCompany>) => Promise<void>;
  onCancel: () => void;
}) {
  const [careerUrl, setCareerUrl] = useState(entry.career_url);
  const [ats, setAts] = useState<ConsultingAts>(entry.ats);
...
export default function ConsultingCompaniesPage() {
  const [entries, setEntries] = useState<ConsultingCompany[]>([]);
  ...
  useEffect(() => {
    getConsultingCompanies()
      .then(setEntries)
      .catch(() => toast.error('Failed to load Consulting list'))
      .finally(() => setLoading(false));
  }, []);

  function upsertLocal(updated: ConsultingCompany) {
    setEntries((prev) => {
  ...
  async function handleAdd(payload: { name: string; career_url: string; ats: ConsultingAts; ats_slug?: string }) {
    setSaving(true);
    try {
      const entry = await addConsultingCompany({ ...payload, active: true });
      upsertLocal(entry);
      setShowForm(false);
  ...
  async function handleSave(id: string, patch: Partial<ConsultingCompany>) {
    try {
      const updated = await updateConsultingCompany(id, {
  ...
  async function handleToggleActive(entry: ConsultingCompany) {
    ...
    if (entry.is_global && entry.active) {
      try {
        const shadow = await disableConsultingCompany(entry.id);
    ...
    try {
      const updated = await updateConsultingCompany(entry.id, { active: !entry.active });
      upsertLocal(updated);
  ...
  async function handleDelete(entry: ConsultingCompany) {
    if (entry.is_global) {
    ...
    if (!confirm(`Remove "${entry.name}" from the Consulting list?`)) return;
    try {
      await removeConsultingCompany(entry.id);
  ...
  return (
    <div className="p-6 max-w-6xl">
      ...
      <div className="flex items-center justify-between mb-6 flex-wrap gap-3">
        <div>
          <h1 className="text-xl font-semibold text-gray-100">Consulting & Outsourcing List</h1>
          <p className="text-sm text-gray-500 mt-0.5">
            Only the {activeCount} active companies on this list are scraped when you click <strong>Scrape Consulting Jobs</strong>.
          </p>
        </div>
        {!showForm && (
          <button
