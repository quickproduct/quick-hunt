...
import {
  getCandidates, getMncJobIds, getMncJobs, getMncJobsCount,
  getJobs, getMncScrapeStatus, setJobHrEmail, triggerMncScrape, updateJobStatus,
  type BulkSendResult, type Candidate, type Job, type MncScrapeStatus,
} from '../../lib/api';
...
export default function MNCJobsPage() {
  // Data
  ...
  const [scrapeStatus, setScrapeStatus]       = useState<MncScrapeStatus | null>(null);
  ...
  const fetchJobs = useCallback(async () => {
    ...
    try {
      ...
      if (err instanceof Error && err.name !== 'AbortError' && err.name !== 'CanceledError') {
        toast.error('Failed to load MNC jobs');
      }
    } finally {
      setLoading(false);
    }
  ...
  // ── Poll MNC scrape status ────────────────────────────────────────────────
  useEffect(() => {
    let cancelled = false;
    const tick = async () => {
      try {
        const s = await getMncScrapeStatus();
        if (cancelled) return;
        setScrapeStatus(s);
      } catch {
  ...
  useEffect(() => {
    ...
    if (wasInFlight && !nowInFlight) {
      fetchJobs();
      toast.success('MNC scrape finished');
    }
  ...
  const handleTriggerScrape = async () => {
    ...
    try {
      await triggerMncScrape(activeCandidateId || undefined);
      toast.success('MNC scrape queued — fanning out per-company tasks');
      // refresh status quickly so the button flips disabled
      try { setScrapeStatus(await getMncScrapeStatus()); } catch {}
    } catch (err: unknown) {
      ...
      if (status === 409) {
        ...
        toast.error('Failed to trigger MNC scrape');
      }
    } finally {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-0.5">
            Software engineer roles scraped directly from top MNC career pages
          </p>
        </div>
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <button
            ...
            title={isScraping ? 'A scrape is already running' : 'Trigger a fresh MNC scrape'}
        ...
        {loading ? (
          ...
          <div className="text-center py-12 space-y-3">
            ...
            <p className="text-gray-400 dark:text-gray-500">
              No MNC jobs yet. Click <strong>Scrape MNC Jobs</strong> to start.
            </p>
          </div>
        ) : (
      ...
      <div className="flex items-center justify-between text-sm text-gray-500 dark:text-gray-400">
        <span>{totalCount.toLocaleString()} MNC job{totalCount !== 1 ? 's' : ''} matching filters</span>
